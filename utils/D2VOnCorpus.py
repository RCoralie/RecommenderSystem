...
def _buildModel(sources, modelPath, nbEpoch=10) :
    ...
    model = Doc2Vec(min_count=1, window=10, size=100, sample=1e-4, negative=5, workers=7, alpha=0.025, min_alpha=0.025, iter=nbEpoch)
...
def createD2VModel():
    ...
    """
    sources = {Config.OVERVIEWS_TR_FILE:'TRAIN_ABSTRACTS'}
    modelPath = Config.OVERVIEW_MODEL
