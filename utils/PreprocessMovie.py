...
from ..utils import apiTMDB, D2VOnCorpus, GloveDict, PreprocessText, PreprocessWord
...
class Preprocessor():
    ...
    def __init__(self, **kwargs):
        ...
        if self.toDo["keywords"] or self.toDo["actors"] or self.toDo["directors"] or self.toDo["titles"]:
            self.dicoGlove = GloveDict.loadGloveDicFromFile()
            self.sizeGloveVector = self.dicoGlove[self.dicoGlove.keys()[0]].shape[0]
        
        if self.toDo["overviews"]:
            self.modelD2V = D2VOnCorpus.loadD2VModel(Config.OVERVIEW_MODEL)
    ...
    def preprocess(self, idMovies):
        ...
        mat, errors= self.preprocessMatrix(idMovies)
    ...
    def preprocessMatrix(self, idMovies):
        ...
        infos, keywords, credits, errors = apiTMDB.requests(idMovies)
    ...
    def overviewProcessingD2V(self, infos):
        ...
        meanMatrixOverview = np.empty([len(infos), D2VOnCorpus.SIZE_VECTOR])  
        
        for i, info in enumerate(infos):            
            overview = apiTMDB.getOverview(info)
            
            if overview == "": 
                meanMatrixOverview[i] = np.zeros(D2VOnCorpus.SIZE_VECTOR)
            else:
                meanMatrixOverview[i] = PreprocessText.textToVect(overview, self.modelD2V)
    ...
    def overviewProcessing(self, infos):
        ...
        for i, info in enumerate(infos):            
            ...
            if overview == "":
                ...
                gArray, wSize = PreprocessWord.wordsToGlove(overview.split(), self.dicoGlove)                
                meanMatrixOverview[i] = PreprocessWord.meanWords(gArray, wSize)
    ...
    def keywordsProcessing(self, moviesKeywords):
        ...
        for i, keyword in enumerate(moviesKeywords):    
            keywords = apiTMDB.getKeywords(keyword)
            gArray, wSize = PreprocessWord.wordsToGlove(keywords, self.dicoGlove)
            
            meanMatrixKeywords[i] = PreprocessWord.meanWords(gArray, wSize)
    ...
    def titlesProcessing(self, infos):
        ...
        for i, info in enumerate(infos):            
            gArray, wSize = PreprocessWord.wordsToGlove(apiTMDB.getTitle(info), self.dicoGlove)
            
            meanMatrixTitles[i] = PreprocessWord.meanWords(gArray, wSize)
    ...
    def ratingProcessing(self, infos):
        ...
        for i, info in enumerate(infos):  
            meanMatrixRating[i] = apiTMDB.getRating(info)/10.0
    ...
    def genresProcessing(self, infos):
        ...
        TMDB_GENRES = apiTMDB.getTmdbGenres()
        ...
        for i, info in enumerate(infos):      
            ...
            for genre in apiTMDB.getGenres(info):
                try:
    ...
    def peopleProcessing(self, moviesCredits, kindOfPeople):
        ...
        for i, credit in enumerate(moviesCredits):
            if kindOfPeople is People.DIRECTOR: names = apiTMDB.getDirectors(credit)
            elif kindOfPeople is People.ACTOR: names = apiTMDB.getActors(credit)
            
            words = []
            for name in names:
                words += name.split()
            
            gArray, wSize = PreprocessWord.wordsToGlove(words, self.dicoGlove)     
            meanMatrixPeople[i] = PreprocessWord.meanWords(gArray, wSize)
    ...
    def compagniesProcessing(self, moviesInfo):
        ...
        for i, info in enumerate(moviesInfo):
            comps = apiTMDB.getProdCompagnies(info)
            ...
            gArray, wSize = PreprocessWord.wordsToGlove(words, self.dicoGlove)     
            meanMatrixCompagnies[i] = PreprocessWord.meanWords(gArray, wSize)
    ...
    def languageProcessing(self, moviesInfo):
        ...
        for i, info in enumerate(moviesInfo):
            langs = apiTMDB.getLanguage(info)
            ...
            gArray, wSize = PreprocessWord.wordsToGlove(words, self.dicoGlove)     
            meanMatrixLanguage[i] = PreprocessWord.meanWords(gArray, wSize)
    ...
    def belongsToProcessing(self, moviesInfo):
        ...
        for i, info in enumerate(moviesInfo):
            meanMatrixBelongs[i] = int( not (apiTMDB.getBelongsTo(info) is None or apiTMDB.getBelongsTo(info) == ""))
    ...
    def runtimeProcessing(self, moviesInfo):
        ...
        for i, info in enumerate(moviesInfo):
            meanMatrixRuntime[i] = apiTMDB.getRuntime(info)
    ...
    def dateProcessing(self, moviesInfo):
        ...
        for i, info in enumerate(moviesInfo):
            meanMatrixDate[i] = apiTMDB.getYear(info)
