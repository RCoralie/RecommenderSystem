...
def createTrainModelDico(mat, labels, iTest = [], iTrain = [], doTest=False):
    ...
    #Train model
    model.fit(matTrain, labelsTrain, batch_size = batch, epochs = epoch, verbose = 1)
...
def buildTestModel(mat, labels, folds):
    ...
    for i, (train, test) in enumerate(skf.split(mat, labels)):
...
def preprocessDataTrainModel(filePath, **params):
    '''
        Preprocess the data from the movies a user has rated
        Builds the model fitting these movies
        
        Params :
            filePath : Path of json file contained data + labels
    ...
    pProcessor = PreprocessMovie.Preprocessor(**params)
